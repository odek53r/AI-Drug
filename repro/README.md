...
| **本包新增** | 寵物資料(48 病 / 158 標籤 / 48 條橋接)、人類 TNBC(23 標籤)、寵物疾病相似度(Wang's method)、機轉融合相似度、Stack 融合層。每筆的來源見 `data_provenance.csv`,版本沿革見 `DATA_VERSIONS.md` |
...
| 檔案 | 內容 |
|---|---|
| `data_provenance.csv` | **唯一的來源紀錄**,282 列 × 29 欄 —— 本包新增的每一筆訓練資料 |
| `DATA_VERSIONS.md` | 資料版本沿革 v1→v4,每版改了什麼、為什麼、驗證結果 |

`data_provenance.csv` 的組成與涵蓋率:

```
drug_disease_label  181   藥-病關聯(寵物 158 + 人類 TNBC 23)
disease_node         49   疾病節點(寵物 48 + 人類 TNBC 1)
disease_bridge       48   寵物↔人類橋接
column / matrix       4   欄位與矩陣層級的改動

282 / 282 列帶識別碼或可點 URL —— 零缺口
```

識別碼欄:`pmid` · `doi` · `nct_id` · `chembl_id` · `mesh_id` · `mondo_id` ·
`venom_id` · `drugbank_id` · `source_url` · `ref_url` · `query_used`(當初用的查詢字串)
...
| ClinicalTrials.gov | 23 | 公眾領域 |
| DrugBank / REDDA | 2 | ⚠️ 學術/商用分流,商用需授權 |

**藥物零新增** —— 894 個 DrugBank ID 與 SMILES 與上游 REDDA 逐列相同。本包只加了
`omics/drug.csv` 的 `Name` 欄(91 個藥名,**91/91 經 PubChem 驗證**:精確相符 89、
縮寫 2、不符 0)。
