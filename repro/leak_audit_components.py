#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""逐成分洩漏稽核:分開量 Az(凍結GNN)/ prop / nmf / full 的 recall@50。
凍結 GNN 是用全部標籤(含測試)訓練的 → 它對 121 個標籤的 recall 就是「記憶/洩漏」上限。
prop/nmf 用 5 折遮蔽 + 硬檢查。另對 drug_sim_fused 做打散負對照。"""
...
print(f"  隨機基準理論值 ≈ 5.6%")
print("-"*66)
print(f"  ① Az 凍結GNN(無遮蔽,見過全部標籤) = {rec_az:5.1f}%   ← 洩漏/記憶上限")
print(f"  ② prop  傳播  (遮蔽) = {rec('prop'):5.1f}%")
print(f"  ③ nmf   分解  (遮蔽) = {rec('nmf'):5.1f}%")
print(f"  ④ prop+nmf   (遮蔽,真·leak-free) = {rec('pn'):5.1f}%")
print(f"  ⑤ full = Az+prop+nmf (Az 未遮→洩漏) = {rec('full'):5.1f}%")
print("-"*66)
print(f"  負對照:prop 用【打散】相似度(遮蔽) = {rec('prop_shuf'):5.1f}%  (應接近隨機→證明訊號來自真相似,非洩漏)")
print("="*66)
print("判讀:")
print(f"  · ④ 才是『測試完全沒被看到』的數字。")
print(f"  · ①遠高於④ → 凍結GNN 記得測試標籤;⑤(含Az)因此被灌水。")
print(f"  · 負對照掉到接近隨機 → prop 的分數是真相似,不是藏了答案。")
